...
claude 用の完成版 Agent は `agents/claude` として同梱しています。消費者は
`agents.Register(claude.New())` で登録するだけで、`--model` / `--resume` / `-n`
の引数組み立てに加え、セッション検出（`SessionDetector`）・復元判定
（`ResumableAgent`）・状態検出（`StateAware`）の任意 IF をまとめて得られます。
自作エージェント（codex 等）を使う場合は同じ IF を実装した Agent を登録してください。
...
- 言語: Go 1.26 系
- テスト: `go test -race ./...`
- Lint: `make lint`（Go ＋ フロント JS をまとめて実行）
    - `make lint-go`: Go 静的解析。[golangci-lint](https://golangci-lint.run/) v2（`staticcheck`・`govet`・`errcheck`・`ineffassign`・`unused` を含む標準構成。設定は `.golangci.yml`）
