...
// detectCatAgentReg は cat を起動しつつ SessionDetector も満たすテスト用 Agent。
// 1 回目の ListSessionIDs（ウォッチャの起動前スナップショット）は preexisting のみ、
// 2 回目以降に new-uuid が出現する「起動後の新規セッション」を決定的に模す。
type detectCatAgentReg struct {
	mu    sync.Mutex
	calls int
}

func (a *detectCatAgentReg) Name() string                                      { return "cat" }
func (a *detectCatAgentReg) Invocation(terminal.RunRequest) (string, []string) { return "cat", nil }
func (a *detectCatAgentReg) ListSessionIDs(string) []string {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.calls++
	if a.calls <= 1 {
...
func TestRegistry_FreshStartFiresSessionListener(t *testing.T) {
	...
	if _, err := r.Start("chat-3", "L", &detectCatAgentReg{}, terminal.RunRequest{}, 100, 40); err != nil {
		t.Fatalf("start: %v", err)
	}
